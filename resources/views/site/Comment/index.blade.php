...
<div class="col-12 text-center">
    <h1 class="text-success">Izohlar ro'yxati</h1>
</div>

<div class="col-12 text-right mb-2">
    <a href="{{route('comments.create')}}" class="btn btn-success btn-sm">Qo'shish</a>
</div>
    ...
    <table id="myTable" class="table table-bordered text-center display">
        ...
        <tbody>
            ...
            <tr>
                ...
                <th>{{$comment->name ?? "Nomsiz"}}</th>
                <th>{{$comment->comment ?? "Mavjud emas"}}</th>
                <td>
                    <img width="40" height="40" src="{{ $comment->image ? asset('/storage/' . $comment->image) : asset('images/m.png') }}" alt="img error">
                </td>
                </th>
                <th>
                    <a class="btn btn-info btn-sm" href="{{route('comments.show', $comment->id)}}">Ko'rish</a>
                </th>
                <th>
                    <a class="btn btn-warning btn-sm" href="{{route('comments.edit', $comment->id)}}">O'zgartirish</a>
                </th>
                <th>
